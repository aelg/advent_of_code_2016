...
void get_key(struct Generator *g, Vector q, int n, int strech){
  int i;
  unsigned char *d;

  while((i = g->n) <= n){
    if(i == g->next_salt_len_increase) ++g->salt_len, g->next_salt_len_increase *= 10;
    int2string(g->end_of_id, i, g->salt_len);
    char buff[32];
    d = MD5((unsigned char*)g->input, g->id_len + g->salt_len, 0);
    for(int b = 0; b < strech; ++b){
    ...
    for(int pos = 0; pos+1 < MD5_DIGEST_LENGTH; ++pos){
      ...
      for(unsigned char match = 0x00; match != 0x10; match += 0x11){
        ...
        if((matchee1 == match && (d[pos+1] >> 4) == single) ||
          ...
          struct Hash h = make_triple(n, to_char[match/0x11]);
          Vector_push(q, &h);
    ...
    for(int pos = 0; pos+2 < MD5_DIGEST_LENGTH; ++pos){
      ...
      for(unsigned char match = 0x00; match != 0x10; match += 0x11){
        ...
        if((matchee1 == match && matchee2 == match && (d[pos+2] >> 4) == single) ||
          ...
          struct Hash h = make_quintuple(n, to_char[single]);
          Vector_push(q, &h);
        }
      }
    }
...
struct Hash get_hash(Vector q, int pos){
...
int solve(int stretch){
  ...
  for(int k = 0; k < 64; ++k){
    int found_key = 0;
      do{
        while(pos >= Vector_size(q))
          get_key(&g, q, i++, stretch);
        struct Hash h = get_hash(q, pos++);
        if(h.t != -1){
          for(int x = pos; ; ++x){
            while(x >= Vector_size(q))
               get_key(&g, q, i++, stretch);
            struct Hash f = get_hash(q, x);
